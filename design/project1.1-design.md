...
برای چک کردن انواع خطاهای ممکن برای انواع اشاره‌گرها، تابعی به نام `check_err` می‌نویسیم که اشاره‌گر، و یک `enum` یا رشته که نشان‌دهندهٔ نوع اشاره‌گر است را ورودی می‌گیرد، و سپس با توجه به اطلاعات بدست‌آمده از `()thread_current` ، مجاز بودن مقادیر اشاره‌گرها را بررسی می‌کند، و در صورت وجود مشکل، با استفاده از `()thread_exit` ریسه از حالت اجرا خارج می‌کند.

در صورت لزوم، مواقعی که منبع موقتی را می‌گیریم، اشاره‌گر مربوطه را در قالب `*void` به انتهای لیستی ‌پیوندی برای هر ریسه می‌افزاییم. سپس پیش از صدا زدن `()thread_exit` در صورت وجود مشکل، و یا قبل از پایان کار فراخوانی‌های سیستم، تابع `()free_all` را صدا می‌زنیم تا حافظهٔ مربوط به تمام اشاره‌گرهای موجود در لیست پیوندی را با استفاده از تابع `()free` آزاد کند.
