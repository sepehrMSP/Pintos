...
برای رسیدگی به خطاهای فراخوانی‌های سیستمی و به طور کلی اطمینان از صحت کارکرد توابع، در ابتدای هر تابع شروطی که هر کدام از آرگومان‌‌ها می‌بایست رعایت کنند را با استفاده از تابعی بنام `check_err` بررسی می‌کنیم که اشاره‌گر را در قالب `* void` به همراه نوع آن در قالب یک `enum` یا رشته ورودی ‌می‌گیرد . کارکرد این تابع همانند `ASSERT` است با این تفاوت که در صورت تشخیص خطا به‌جای `PANIC` ، به کمک `()thread_exit` به کار ریسه پایان می‌دهد.
