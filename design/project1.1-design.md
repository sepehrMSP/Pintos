...
با توجه به اینکه ریسه‌ها و همگام‌سازی بین آنها در این سیستم‌عامل به درستی انجام نمی‌شود، به جهت جلوگیری از شرایط مسابقه، از تابع `strtok_r` استفاده می‌شود. درواقع پیاده‌سازی `strtok` به صورت زیر است:
...
`files`: لیستی از فایل‌هایی است ریسه باز کرده است. هر عنصر آن اشاره‌گری به چیزی از جنس `thread_file` خواهد بود.
`locks`: لیستی از قفل‌هایی که ریسه در حال حاضر در دست دارد.
...
این متغیر را همانطور که در مستند آمده است برای برقراری همگام‌سازی بین اعمال مربوط به پرونده‌ها در نظر گرفتیم. (متغیر سراسری است)
...
در داده‌ساختاری که در بالا آوردیم یک قسمت از آن مربوط به توصیف‌کننده پرونده بود و از این طریق ما توصیف‌کننده‌های پرونده‌ها را در کد ذخیره‌ می‌کنیم. چون خودمان تعیین می‌کنیم که در خروجی توابع مربوط به پرونده‌ها چه توصیف‌کننده‌ای برمی‌گردانیم، توصیف‌کننده‌ها را در سطح پردازه یکتا نگه می‌داریم.
...
ابتدا اشاره‌گری را که کاربر به ما داده است چک می‌کنیم که مقدار معتبری داشته باشد. برای انجام اینکار ابتدا با کمک `pagedir_getpage()` صفحه مربوط به این آدرس را در صورت وجود بدست می‌آوریم (باید اشاره‌گر کاربر معتبر و غیر پوچ باشد) سپس مقادیر را از کرنل به حافظه کاربر یا برعکس (بسته به اینکه قصد خواندن یا نوشتن داریم) کپی می‌کنیم پس از اتمام اطلاعات این صفحه در صورت نیاز به صفحه بعدی، دوباره ابتدا از تابع `pagedir_getpage()` بهره می‌بریم.
...
جواب برای هر دو سوال بیشترین دو و کمترین یک بار است (بسته به نیاز). و این مقادیر قابل بهبود نیستند. چرا که بدون بررسی جدول صفحات امکان دسترسی به داده‌ها وجود ندارد. اما اینکه یکبار نیاز است یا دوبار نیاز است را می‌توانیم همان ابتدا و از روی آدرس و اندازه کپی متوجه شویم.
...
برای رسیدگی به خطاهای فراخوانی‌های سیستمی و به طور کلی اطمینان از صحت کارکرد توابع، در ابتدای هر تابع شروطی که هر کدام از آرگومان‌‌ها می‌بایست رعایت کنند را با استفاده از تابعی بنام `check_err` بررسی می‌کنیم که اشاره‌گر را در قالب `* void` به همراه نوع آن در قالب یک `enum` یا رشته ورودی ‌می‌گیرد . کارکرد این تابع همانند `ASSERT` است با این تفاوت که در صورت تشخیص خطا به‌جای `PANIC` ، به کمک `()thread_exit` به کار ریسه پایان می‌دهد. توجه کنید که در صورتی که نیاز باشد همزمانی قفل‌ها رسیدگی شود می‌توانیم از لیست قفل‌هایی که هم اکنون ریسه در دسترس دارد استفاده کنیم و در محل مناسب کد (می‌تواند در `thread_exit` یا جای دیگری باشد) این قفل‌ها را آزاد کنیم، مثلا حالتی را در نظر بگیرید که یک ریسه در حین اجرای خود چند قفل متفاوت برای اشیا گوناگون را در دست بگیرد و بعد از مدتی به هر دلیلی اجرای ریسه با خطا مواجه شود، در اینصورت می‌بایست تمامی قفل‌هایی که در اختیار ریسه بودند آزاد شوند تا دچار حالت قفل مرده در کد نشویم.

`check_err(void *arg, char *type, condition)`

با توجه به اینکه در شرایط کنونی حافظه‌ٔ موقتی را تخصیص نمی‌دهیم، تنها در مواقعی که اشاره‌گر نامعتبر داشته باشیم، کافی است که تابع `()thread_exit` را صدا بزنیم و صفحه مربوط به ریسه آزاد می‌شود.

در حالت کنونی مثالی برای گرفتن حافظه در توابع سیستمی نداریم، اما در صورت نیاز با استفاده از داده‌ساختار زیر متغیر‌هایی که در حافظه گرفته‌ایم را به صورت یک لیست پیوندی برای هر ریسه نگه می‌داریم تا در صورت وقوع خطا با حرکت روی این لیست تمامی متغیر‌ها را آزاد کنیم:
...
برای اینکه درگیر جزییات آدرس‌دهی‌های متفاوت هسته و کاربر نشویم (هنگام خواندن/نوشتن) (چراکه آدرس‌ها در کاربر نگاشته شده هستند و آدرس واقعی نیستند)  و صرفا صحت اشاره‌گر حافظه کاربر را بررسی کنیم، این روش را انتخاب کردیم. چون دو کار خواندن/نوشتن و تشخیص خطا از یکدیگر جدا‌ می‌کند، و دیگر نیازی به دست بردن در تابع `page_fault` که پیچیدگی‌های زیادی دارد (به علت سطح پایین بودن) نخواهد بود.


> طراحی شما برای توصیف‌کننده‌های فایل چه نقاط قوت و ضعفی دارد؟
مزیت این حالت (که در سطح ریسه یکتا باشد) نسبت به حالتی که توصیف‌کننده‌های فایل‌ها را در سطح سیستم‌عامل یکتا بگیریم، این است که راحت‌تر می‌توان بررسی کرد که هر ریسه به چه فایل‌هایی دسترسی دارد، و همچنین می‌توان آن‌ها را در حافظهٔ هر پردازه نگه داشت، اما نقطهٔ ضعف این حالت، نیاز به حافظهٔ بیشتر است.

> در حالت پیش‌فرض نگاشت `tid` به `pid` یک نگاشت همانی است. اگر این را تغییر داده‌اید، روی‌کرد شما چه نقاط قوتی دارد؟
با توجه اینکه هر پردازه یک ریسه دارد فعلا نیازی به تمایز قائل شدن برای آنها نیاز نمی‌باشد.
